...
void lemlib::Chassis::swingToPoint(float x, float y, DriveSide lockedSide, int timeout, SwingToPointParams params,
                                   bool async) {
    if (!async)
        lemlib::infoSink()->debug("Call to: swingToPoint(x:{}, y:{}, side:{}, t:{}, minSpd:{}, async:{})", 
                          x, 
                          y, 
                          static_cast<int>(lockedSide), // Print the enum value
                          timeout, 
                          params.minSpeed,              // Print a member of the struct
                          async);
